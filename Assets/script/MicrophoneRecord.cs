...
// ReSharper disable RedundantCast
    ...
    // Add these new fields to your MicrophoneRecord class:
    private int _voiceDetectedStartPos = -1; // Position when voice was first detected
    private bool _hasVoiceBeenDetected = false; // Flag to track if voice was ever detected
    [Tooltip("Buffer time in seconds to include before voice detection starts")]
    public float voiceStartBufferSec = 0.5f; // Buffer to include before voice detection
    ...
    // Modify the UpdateVad method to track when voice is first detected:
    private void UpdateVad(int micPos)
    {
        if (!useVad)
            return;
        ...
        if (vad != IsVoiceDetected)
        {
            // Track when voice is first detected
            if (vad && !_hasVoiceBeenDetected)
            {
                _voiceDetectedStartPos = micPos;
                _hasVoiceBeenDetected = true;
                Debug.Log($"Voice first detected at position: {_voiceDetectedStartPos}");
            }
    ...
    // Modify StartRecord to reset voice detection tracking:
    // Modify StartRecord to reset voice detection tracking:
    public void StartRecord()
    {
#if UNITY_ANDROID && !UNITY_EDITOR
    ...
    if (!Permission.HasUserAuthorizedPermission(Permission.Microphone))
    {
        ...
        // Reset voice detection tracking
        _voiceDetectedStartPos = -1;
        _hasVoiceBeenDetected = false;
    }
    ...
    // Modify StopRecord to use voice-trimmed audio:
    public void StopRecord(float dropTimeSec = 0f)
    {
        if (!IsRecording)
            return;

        // Get data from voice detection start (if voice was detected) or full buffer
        var data = GetMicBuffer(dropTimeSec, fromVoiceStart: _hasVoiceBeenDetected);
        ...
        var startDescription = _hasVoiceBeenDetected ?
            $"from voice detection start (with {voiceStartBufferSec}s buffer)" :
            "from recording start";
        Debug.Log($"Stopped microphone recording {startDescription}. Final audio length " +
                             $"{finalAudio.Length} ({finalAudio.Data.Length} samples)");
        ...
        // play echo sound (using full original data for echo, not trimmed)
        if (echo)
        {
            var fullData = GetMicBuffer(dropTimeSec, fromVoiceStart: false);
            var echoClip = AudioClip.Create("echo", fullData.Length,
                _clip.channels, _clip.frequency, false);
            echoClip.SetData(fullData, 0);
            PlayAudioAndDestroy.Play(echoClip, Vector3.zero);
        }

        // finally, fire event with voice-trimmed audio
        OnRecordStop?.Invoke(finalAudio);
    }
    ...
    // Modify GetMicBuffer to support starting from voice detection point:
    private float[] GetMicBuffer(float dropTimeSec = 0f, bool fromVoiceStart = false)
    {
        var micPos = Microphone.GetPosition(RecordStartMicDevice);
        var len = GetMicBufferLength(micPos);
        if (len == 0) return Array.Empty<float>();
        ...
        int startPos = 0;
        if (fromVoiceStart && _hasVoiceBeenDetected && _voiceDetectedStartPos >= 0)
        {
            // Add buffer before voice detection point
            var bufferSamples = (int)(_clip.frequency * voiceStartBufferSec);
            var adjustedStartPos = _voiceDetectedStartPos - bufferSamples;
            ...
            if (adjustedStartPos < 0)
            {
                if (_madeLoopLap)
                {
                    ...
                    adjustedStartPos = ClipSamples + adjustedStartPos;
                }
                else
                {
                    // Can't go before recording start, use 0
                    adjustedStartPos = 0;
                }
            }

            startPos = adjustedStartPos;
            // Recalculate length from adjusted start to current position
            len = GetMicPosDist(adjustedStartPos, micPos);
        }
        else if (_madeLoopLap)
        {
            startPos = micPos;
        }
        ...
        // get audio data from calculated start position
        var data = new float[len];
        _clip.GetData(data, startPos);
